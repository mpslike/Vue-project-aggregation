<template>
    <div>
        <p>
            根据当前筛选条件：共{{$store.state.filtercar.count}}个结果，当前{{$store.state.filtercar.pageinfo.page}}页
        </p>
        <Table :columns="tablecols" :data="tabledata"  @on-sort-change="sortchange"></Table>
        <div class="page">
            <Page :total="count" show-sizer @on-change="pagechange" @on-page-size-change="pagesizechange"></Page>
        </div>
    </div>
</template>
